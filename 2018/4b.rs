...
fn main() {
    ...
    let mut handle = stdin.lock();
  	...
  	for l in seq {
		...
		//println!("{:?}", total);
		let (maxidx, maxmin, maxsleep) = state.iter().map(
			|(i, &a)| {
				let (min, sleep) = a.iter().enumerate().max_by_key(|&(_, v)| v).unwrap();
				return (i, min, *sleep);
			}).max_by_key(|&(_, _, v)| v).unwrap();
 	  println!("{} {} {}", maxidx, maxmin, maxsleep);
