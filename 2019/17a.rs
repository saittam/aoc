...
fn main() {
  ...
  let mut suma = 0; 
  for (y, row) in state.iter().enumerate().skip(1) {
    for (x, t) in row.iter().enumerate().skip(1) {
      if *t == Tile::Scaffold {
        if [(x, y - 1), (x, y + 1), (x - 1, y), (x + 1, y)].iter().all(
          |(px, py)| state.get(*py).and_then(|r| r.get(*px)) == Some(&Tile::Scaffold)) {
          suma += x * y;
        }
      }
    }
