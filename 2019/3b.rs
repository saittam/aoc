...
fn main() {
  ...
  let mut r = std::collections::HashMap::<Pos, usize>::new();
  for (c, p) in pos_iterator(first.iter()).enumerate() {
    r.entry(p).or_insert(c + 1);
  }
  
  let m = pos_iterator(second.iter())
    .enumerate()
    .filter_map(|(c2, p)| r.get(&p).map(|c1| c1 + c2 + 1))
    .min().unwrap();
