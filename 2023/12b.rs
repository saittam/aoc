...
fn count<'a>(
  m: &mut HashMap<(&'a str, Option<u64>, &'a [u64]), u64>,
  pat: &'a str,
  open: Option<u64>,
  seq: &'a [u64])
  -> u64 {
  ...
  let scount = |m| seq.first()
    .map(|n| count(m, pat, Some(n - 1), &seq[1..]))
    .unwrap_or(0);
...
fn main() {
  ...
  let n = rows.iter()
    .map(|(p, s)| {
      let p = p.chars().chain(['?'])
        .cycle().take(5 * p.len() + 4)
        .collect::<String>();
      let s = s.iter().copied().cycle().take(5 * s.len())
        .collect::<Vec<_>>();
      count(&mut HashMap::new(), &p, None, &s)
    })
    .sum::<u64>();
